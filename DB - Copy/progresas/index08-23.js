      ...
      case 2:
        {
          ...
          if (vardas.trim() !== "" && pavarde.trim() !== "" && isFinite(alga)) {
            try {
              let r = await dbQuery(conn, "insert into zmones (vardas, pavarde, gim_data, alga) values (?, ?, ?, ?)", [vardas, pavarde, gim_data, alga]);
      ...
      case 3:
        {
          ...
          if (isFinite(id) && vardas.trim() !== "" && pavarde.trim() !== "" && isFinite(alga)) {
            try {
              let r = await dbQuery(conn, "update zmones set vardas = ?, pavarde = ?, gim_data = ?, alga = ? where id = ?", [vardas, pavarde, gim_data, alga, id]);
            } catch (err) {
              console.log("Klaida: ", err);
            }
          } else {
